...
function ShoppingCart({navigation}) {
    ...
    const minusActionQty = (id) => {
        ...
        let objIndex = myArray.findIndex((obj => obj.guid == id));
            ...
            <VStack>                            
                <HStack justifyContent="space-between" ml={5} mr={5}>
                    ...
                    <VStack w='40%' justifyContent='space-between' alignItem='flex-start'>
                        ...
                        <Text style={{fontFamily:'Inter_700Bold'}} fontWeight='bold' fontSize="md" _dark={{ color: "warmGray.50"}} color="coolGray.800" alignSelf="flex-start">{value == 'two'? item.cart_product.price_mm && item.cart_product.price_mm.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : item.cart_product.price && item.cart_product.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</Text> 
                        </HStack>
                    </VStack>
                    <Spacer />
                    <VStack justifyContent='space-between' maxW='20%' alignItems='flex-end'>
                        <TouchableOpacity style={{padding:10}} onPress={()=>removeAction(item.guid)}>
                            <CloseIcon size="3" />
                        </TouchableOpacity>
                        <View  style={styles.itemCount}>
                        <HStack justifyContent='space-evenly' alignContent='center'alignItems='center'> 
                            <TouchableOpacity onPress={() => plusActionQty(item.guid)} style={{padding:10}}>
                            ...
                            <TouchableOpacity onPress={() => minusActionQty(item.guid)} style={{padding:10}}>
